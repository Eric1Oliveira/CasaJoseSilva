...
async function handleCheckout() {
  ...
  try {
    ...
    scheduleAutoPreparingStatus(createdOrder?.id, currentUser.id);
    await trackPromotionConversion(appliedCoupon, createdOrder?.id || null).catch(() => {});
    if (appliedCoupon?.id) await increaseCouponUsage(appliedCoupon.id);
...
function updateOrderCardRealtime(updatedOrder) {
  ...
  if (orderCard) {
    const newTimeline = buildOrderTimeline(updatedOrder.status);
    const timelineContainer = orderCard.querySelector('.order-timeline');
    if (timelineContainer) {
      timelineContainer.outerHTML = newTimeline;
